...
public:
	...
	void createPojectionPerspMatrix(float fov, float aspect, float n, float f) {
		...
		Mat.Direct._11 = xScale;
		Mat.Direct._22 = yScale;
		Mat.Direct._33 = (f + n) / nearmfar;
		Mat.Direct._43 = -1;
		Mat.Direct._34 = 2 * f * n / nearmfar;		
	}

	void createPojectionOrthoMatrix(float left, float right, float bottom, float top, float nearPlane, float farPlane) {

		memset(Mat.m, 0x00, 16 * sizeof(float));
		Mat.Direct._11 = 2.0f / (right-left);
		Mat.Direct._22 = 2.0f / (top - bottom);
		Mat.Direct._33 = 2.0f / (farPlane - nearPlane);
		Mat.Direct._44 = 1;

		Mat.Direct._14 = -(right + left) / (right - left);
		Mat.Direct._24 = -(top + bottom) / (top - bottom);
		Mat.Direct._34 = -(farPlane + nearPlane) / (farPlane - nearPlane);
