...
glDisable(GL_DEPTH_TEST);
		...
		setProjectionOrtho(0, ScreenWidth, ScreenHeight, 0, -1.0f, 2.0f);
	...
	void rotationModel(float angleDeg, float x, float y, float z) {
		YMat44 m;
		YVec3f v(x, y, z);
		m.createRotateAxe(v.normalize(), (angleDeg * (float)M_PI * 2.0f) / 360.0f);
		MatM *= m;
	}
	...
	void pushModel() {
		modelStack.push(MatM);
	}

	void setProjectionPersp(float fov, float aspect, float n, float f) {
		MatP.createPojectionPerspMatrix(fov, aspect, n, f);
	}

	void setProjectionOrtho(float left, float right, float bottom, float top, float nearPlane, float farPlane) {
		MatP.createPojectionOrthoMatrix(left, right, bottom, top, nearPlane, farPlane);
	}

	void setViewMatix(YVec3<float>& pos, YVec3<float>& lookat, YVec3<float>& up) {
		MatV.createViewMatrix(pos, lookat, up);
	}
